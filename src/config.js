...
// Browser API compatibility shim for Firefox (browser.*) & Chrome (chrome.*)
const pdBrowser = (typeof chrome !== 'undefined' && chrome.runtime) ? chrome : (typeof browser !== 'undefined' ? browser : null);

/**
 * Universal Storage Get helper supporting Chrome callbacks & Firefox Promises.
 */
function pdStorageGet(keys, callback) {
  ...
  try {
    let called = false;
    const res = api.storage.local.get(keys, (items) => {
      if (!called) {
        called = true;
...
/**
 * Universal Storage Set helper supporting Chrome callbacks & Firefox Promises.
 */
function pdStorageSet(items, callback) {
  const api = (typeof chrome !== 'undefined' && chrome.storage) ? chrome : (typeof browser !== 'undefined' ? browser : null);
  if (!api || !api.storage || !api.storage.local) {
    if (callback) callback();
    return;
...
/**
 * Universal Storage Change Listener helper.
 */
function pdStorageOnChanged(listener) {
  const api = (typeof chrome !== 'undefined' && chrome.storage) ? chrome : (typeof browser !== 'undefined' ? browser : null);

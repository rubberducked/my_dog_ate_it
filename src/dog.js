...
(function () {
  ...
  function setSprite(wrapper, img, assetPath) {
    ...
    try {
      const fullUrl = pdBrowser.runtime.getURL(assetPath) + '#t=' + Date.now();
      img.src = fullUrl;
    } catch (_) {
  ...
  // --------------------------------------------------------------------------
  function boot() {
    pdBrowser.storage.local.get(PD_DEFAULTS, (items) => {
    ...
    pdBrowser.storage.onChanged.addListener((changes, area) => {

...
(function () {
   ...
   * Removes 20 to 25 characters using native prototype value setters.
   */
  function biteInputOrTextarea(el) {
    ...
    const biteMin = Math.min(val.length, minTarget);
    const biteMax = Math.min(val.length, maxTarget);
    const biteLen = Math.floor(Math.random() * (biteMax - biteMin + 1)) + biteMin;

    if (biteLen <= 0) return null;

    const start = Math.floor(Math.random() * (val.length - biteLen + 1));
    const eatenText = val.slice(start, start + biteLen);
    const newVal = val.slice(0, start) + val.slice(start + biteLen);

    // Caret preservation math
    let selStart = el.selectionStart;
    let selEnd = el.selectionEnd;
    if (typeof selStart === 'number' && typeof selEnd === 'number') {
      if (selStart > start + biteLen) {
        selStart -= biteLen;
      } else if (selStart > start) {
        selStart = start;
      }

      if (selEnd > start + biteLen) {
        selEnd -= biteLen;
  ...
  /**
   * Destructive bite for contenteditable elements (20-25 characters).
   */
  function biteContentEditable(el) {
    ...
    const biteMin = Math.min(nodeLen, minTarget);
    const biteMax = Math.min(nodeLen, maxTarget);
    const biteLen = Math.floor(Math.random() * (biteMax - biteMin + 1)) + biteMin;

    if (biteLen <= 0) return null;

    const start = Math.floor(Math.random() * (nodeLen - biteLen + 1));
    const eatenText = node.data.slice(start, start + biteLen);

    node.deleteData(start, biteLen);
  ...
  function biteCustomEditor(el) {
    ...
    // 2. Strategy A: document.execCommand('delete')
